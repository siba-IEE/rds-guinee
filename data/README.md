...
- `nasa-power/` : NASA POWER, mensuel 1981-2025 et journalier 1981-2025 (un
  fichier par décennie). Les deux séries incluent la précipitation.
- `horaire/` : conventions d'accès aux séries horaires. Le volume horaire est
  publié en archive de release, pas dans le dépôt (voir `horaire/README.md`).
...
| `instant_utc` | pas des séries horaires, ISO 8601, début de pas, UTC | |
| `ghi` | irradiation globale horizontale | kWh/m²/jour ; Wh/m² au pas horaire |
| `dni` | irradiation directe normale | kWh/m²/jour ; Wh/m² au pas horaire |
| `dhi` | irradiation diffuse horizontale | kWh/m²/jour ; Wh/m² au pas horaire |
...
| `precipitation` | précipitation corrigée (NASA POWER PRECTOTCORR) | mm/jour ; mm/h au pas horaire |
...
- **Cellules vides** : valeur absente à la source. Pour NASA POWER, la météo
  remonte à 1981, `ghi` et `dhi` à 1984, et `dni`, `kt`, `albedo` à 2001
  (début des observations CERES), au mensuel comme au journalier. Pour CAMS,
  la `dni` débute en février 2004 (début du service CAMS Radiation).
